namespace KappaAIO.Champions
{
    ...
    internal class Brand : Base
    {
        ...
        static Brand()
        {
            try
            {
                Q = new Spell.Skillshot(SpellSlot.Q, 1000, SkillShotType.Linear, 250, 3000, 100);
        ...
        {
          CurrentTarget = TargetSelector.GetTarget(W.Range, DamageType.Magical);
          var minion = EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy).Where(a => a.Distance(Player.Instance) < E.Range && a.Distance(CurrentTarget.ServerPosition) < 250 && a.brandpassive()).OrderBy(a => a.Distance(CurrentTarget.ServerPosition)).FirstOrDefault();
