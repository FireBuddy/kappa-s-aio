namespace KappaAIO.Champions
{
    ...
    internal class Brand : Base
    {
        ...
        {
          CurrentTarget = TargetSelector.GetTarget(W.Range, DamageType.Magical);
          var minion = EntityManager.MinionsAndMonsters.GetLaneMinions().Where(a => a.Distance(Player.Instance) < E.Range && a.Distance(CurrentTarget < 150)).OrderBy(a => a.Health).FirstOrDefault();
        }
