namespace KappaAIO.Champions
{
    ...
    internal class Brand : Base
    {
        ...
        {
          CurrentTarget = TargetSelector.GetTarget(W.Range, DamageType.Magical);
          var minion = EntityManager.MinionsAndMonsters.GetLaneMinions().Where(a => a.Distance(Player.Instance) < E.Range && a.Distance(CurrentTarget.ServerPosition) < 150 && a.brandpassive()).FirstOrDefault(a => a.Distance(CurrentTarget.ServerPosition) < 150);
